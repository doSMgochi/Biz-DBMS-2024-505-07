...
SYTSDBA 는 Oracle DBMS 의 매우 중요한 권한을 나타낸다.
SYSDBA는 DB 를 생성하고, 제거하고, 사용자를 생성하고, 제거하고, 여러가지 시스템관리를 할 수 있는 권한이다.
따라서 SYS 사용자의 비밀번호는 잘 관리되어야 한다.
*/
-- Oracle DBMS 프로젝트를 처음 시작할 때는 관리자로 접속하여 TableSpace 와 User 를 생성해 주어야 한다.
...
SIZE 1M -- Data 저장소의 공간을 1MByte 만큼 예약하겠다. SIZE 1MB 처럼 사용하면 안 된다.
...
-- 사용자 생성
/*
오라클 12c 이후 버전에서 user 를 생성할 때 일반적인 문자열만으로 user 를 생성할 수 없다
사용자 이름 앞에 반드시 C## 이 부착된 형태로 user name 을 생성해야한다.
이 설정은 여러가지 불편함이 발생하고
특히 JAVA 라던가 다른 곳에서 DBMS 에 접속하는 프로그래밍을 수행할 때는 문제를 일으킬 수 있다.
*/
...
/*
USER 를 생성할 때 앞에서 생성한 TableSpace 와 USER 계정을 연결해주어야만
사용자 데이터가 새로 생성한 TableSpace 에 저장된다.
*/
CREATE USER user1 IDENTIFIED BY 1234;
-- 새로 생성된 user1 은 아무런 권한이 없어서, 로그인 조차 할 수 없다.
-- user1 에게 여러가지 권한을 부여해야만 DBMS 에 접근하여 업무를 수행할 수 있다.
/*
권한 부여 정책
생성된 USER(user1) 에게 로그인, 테이블 생성, CRUD, 기타 여러가지 권한을 일일히 부여하는 것이 실무상 원칙
학습하는 입장에서, 로컬에 저장된 데이터에 접근한다는 제한적인 환경에서 여러가지 권한을 일일히 부여하는 것은
상당히 불편함과 어려움이 있다.

여기서는 user1 에게 DBA 권한을 부여할 것이다.
오라클에서 DBA 권한은 SYSDBA 보다 조금 제한된 권한이며, 로그인, 테이블 생성 제거, CRUD 등의 업무를
모두 수행할 수 있는 권한이다.
*/
